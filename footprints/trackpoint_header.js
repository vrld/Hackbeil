module.exports = {
  ...
  body: p => {
    ...
    const text = net_names.map((name, i) =>
      `(fp_text user "${name}" (at -2.5 ${x(i)} ${p.rot}) (layer F.SilkS) (effects (font (size .8 .8) (thickness .15))))
       (fp_text user "${name}" (at -2.5 ${x(i)} ${p.rot}) (layer B.SilkS) (effects (font (size .8 .8) (thickness .15)) (justify mirror)))`
    )

    const pads = net_names.map((name, i) => `(pad ${i + 1} thru_hole ${i == 0 ? 'rect' : 'circle'} (at 0 ${x(i)} ${p.rot}) (size 1.5 1.5) (drill 0.8) (layers *.Cu *.SilkS *.Mask) ${p[name].str})`)
